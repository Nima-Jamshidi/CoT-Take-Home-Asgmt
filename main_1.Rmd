...
## Parking Tickets
Here I wrote a function for getting data for any year available on the website. There are a few note:
- `opendatatoronto` package has problem with pulling `.csv` files. This is done manually here.
- For some of the years, the `.csv` file is corrupt. The Unicode requires adjustment. This function does that.
...
```{r parking ticket API 1,eval = FALSE}
...
```{r parking ticket API 2, warning = FALSE, eval = FALSE}
...
PaTi_data_top20 <- PaTi_data_top20_1 %>%
  ...
  arrange(desc(n))
```
The One Address Repository has grouped addresses in `LINK`s that are street blocks. I use these LINKs to calculate the most common location for the top Infraction codes. Here you can see the map of the most common location for top Infraction codes. On an html-based veiwer you can hover over the map and click for more information about each locatio and infraction code.

```{r Q1 plot}
fine_data <- PaTi_data_tot %>% group_by(infraction_code,set_fine_amount) %>% summarise(n=n()) %>% filter(n == max(n)) %>% select(-n)
...
PaTi_GP_tile <- PaTi_GP %>%
  ggplot()+
  geom_raster(aes(x=factor(infraction_code,levels = PaTi_data_top20$infraction_code),y=GP_distance,fill=no_GP))+ 
  theme_bw()+
  ...
  scale_y_continuous(name = "Green P Parking lot distance from\nthe most common location for an infraction code (m)",expand=c(0,0))+
  scale_fill_continuous(name = "number of\nGreen P\nparking lots")
  
plot(PaTi_GP_tile)
```
Here you can see the map of common infraction locations with their closest Green P parking connected to them with a line. On an html-based veiwer you can hover and click on the line, Green P Parking to gain more information.

```{r Green P map}
PaTi_GP_over_id <- function(r) {
  GP_sf_buf <- GP_sf %>%
    st_transform(2952) %>%
    st_buffer(dist = r) %>%
    st_transform(4326)
  ...
  res <- unique(c(unlist(sparse_pl),unlist(sparse_pt)))
  
  return(res)
}

PaTi_GP_400_id <- PaTi_GP_over_id(400)
GP_sf_buf <- GP_sf[PaTi_GP_400_id,] %>%
    st_transform(2952) %>%
    st_buffer(dist = 400) %>%
    st_transform(4326)

PaTi_GP_400_plot <- leaflet() %>% 
  addProviderTiles(providers$CartoDB.Positron) %>% 
  addPolygons(data = PaTi_polygon_pl,
              popup=paste(#"<b>Infraction Code</b>",PaTi_polygon_pl$infraction_code,"<br>",
                          "<b>Description:</b>",PaTi_polygon_pl$infraction_description,"<br>",
  ...
  addPolygons(data = PaTi_polygon_pt,
              ...
              label = ~ paste0("Infraction Code ", infraction_code),
              color = "#6e031e") %>% 
  addPolygons(data = GP_sf_buf,
              popup=paste(
                          "<b>Address:</b>",GP_sf_buf$address,"<br>",
                          "<b>Rate:</b>",GP_sf_buf$rate),
...
PaTi_polygon_pl <- PaTi_polygon_pl %>% left_join(PaTi_polygon_center %>% select(closest_GP,GP_dist,infraction_code,color) %>% st_drop_geometry())

PaTi_polygon_pt <- PaTi_polygon_pt %>% left_join(PaTi_polygon_center %>% select(closest_GP,GP_dist,infraction_code,color) %>% st_drop_geometry())
...
PaTi_GP_map <- leaflet() %>% 
  ...
  addMarkers(data = GP_sf_closest,
             # icon = makeIcon("https://www.iconpacks.net/icons/2/free-location-icon-2955-thumb.png", iconWidth = 16, iconHeight = 16),
...
ngbh_retrieve <- function(URL,No,format) {
  ...
  res <- st_read(files[str_ends(files, ".shp")])
...
ngbh_shp <- ngbh_retrieve("https://open.toronto.ca/dataset/neighbourhoods/",5,"zip")
ngbh_prof_metadata <- opendatatoronto::show_package("https://open.toronto.ca/dataset/neighbourhood-profiles/")
ngbh_prof_list <- opendatatoronto::list_package_resources("https://open.toronto.ca/dataset/neighbourhood-profiles/")
ngbh_prof_data <- ngbh_prof_list[2,] %>% get_resource()
GeoC_shp

(ngbh_prof_data) %>% filter(Category == "Mobility")

(ngbh_prof_data) %>% filter(X_id == 2370) %>% pivot_longer(cols = City.of.Toronto:Yorkdale.Glen.Park,
                                  values_to = "value",
                                  names_to = "neighbourhood") %>% 
  filter(!is.na(str_match(neighbourhood,"Annex")))
str_match(c("jasds","hjds"),"a")
(ngbh_prof_data %>% filter(Category != "Language"))[1000:1575,]
(ngbh_prof_data %>% select(Characteristic,X_id,Category) %>% filter(Category != "Language"))[1000:1575,]
ngbh_prof_data %>% filter(X_id==947)
ngbh_prof_data %>% filter(Category == "Income") %>% select(Characteristic,X_id,Category,-Data.Source,-Topic,everything())
```


```{r neighbourhoods API}

sparse_neigh <- PaTi_polygon_center %>% st_intersects(ngbh_shp)
is.na(sparse_neigh) <- lengths(sparse_neigh) == 0
PaTi_polygon_center <- PaTi_polygon_center %>% ungroup() %>% mutate(neighbourhood = ngbh_shp[unlist(sparse_neigh),]$AREA_NAME)
PaTi_polygon_center <- PaTi_polygon_center %>% mutate(neighbourhood = str_remove(neighbourhood,"\\(\\d+\\)"),
                               neighbourhood = str_replace_all(neighbourhood,"([:alpha:]+)","\\1\\."),
                               neighbourhood = str_remove_all(neighbourhood,"[^a-zA-Z\\.]"),
                               neighbourhood = str_remove_all(neighbourhood,"\\.$"),
                               )
...
ngbh_filtered_data_wide <- ngbh_filtered_data_wide %>% mutate(High_infraction = neighbourhood %in% PaTi_polygon_center$neighbourhood)
