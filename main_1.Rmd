...
```{r parking ticket query 1,eval = FALSE}
...
```{r parking ticket query 2, warning = FALSE, eval = FALSE}
...
```{r parking ticket query 3, warning = FALSE}
...
PaTi_GP_over <- function(r) {
  ...
  GP_sf_buf %>% st_intersects(PaTi_polygon_pl)
  sparse_pl <-
  ...
  GP_sf_buf %>% st_intersects(PaTi_polygon_pt)
  sparse_pt <-
...
PaTi_GP_tile <- PaTi_GP %>%
  ...
  scale_y_continuous(name = "Green P Parking lot distance from\nthe most common location for an infraction code",expand=c(0,0))+
...
```{r Green P map}
dist_mat <- GP_sf %>% st_distance(PaTi_polygon_pl %>% st_centroid())
PaTi_polygon_pl <- PaTi_polygon_pl %>% 
  ungroup() %>%
...
dist_mat <- GP_sf %>% st_distance(PaTi_polygon_pt %>% st_centroid())
PaTi_polygon_pt <- PaTi_polygon_pt %>% 
  ungroup() %>%
  mutate(row_num = row_number()) %>% 
  mutate(
    closest_GP = map_dbl(row_num, ~ which.min(dist_mat[,.x])),
...
PaTi_polygon_pl[,c("closest_GP","GP_dist")]
GP_sf_closest <- GP_sf %>% inner_join(rbind(PaTi_polygon_pl[,c("closest_GP","GP_dist")],PaTi_polygon_pt[,c("closest_GP","GP_dist")]) %>% st_drop_geometry(),by = c("id"="closest_GP"))

GP_sf_closest_buf <- GP_sf_closest %>% st_transform(2952)
GP_polygons <- map2(GP_sf_closest_buf$geometry,GP_sf_closest_buf$GP_dist, ~st_buffer(.x,.y))

GP_sf_closest_buf <- GP_sf_closest_buf %>% 
  st_drop_geometry() %>%
  mutate(row_num = row_number()) %>%
  mutate(geometry = map(row_num, ~ tt[[.x]])) %>% 
  st_as_sf(sf_column_name = "geometry",crs = 2952) %>% 
  mutate(color = paletteer_c("grDevices::rainbow", 20)) %>% 
  st_transform(4326)

PaTi_polygon_pl <- PaTi_polygon_pl %>% left_join(GP_sf_closest_buf %>% st_drop_geometry() %>% select(id,color),by = c("closest_GP" = "id"))

PaTi_polygon_pt <- PaTi_polygon_pt %>% left_join(GP_sf_closest_buf %>% st_drop_geometry() %>% select(id,color),by = c("closest_GP" = "id"))
...
ngbh_shp %>% filter(str_(AREA_NAME,"Etobicoke"))
str_which(ngbh_shp$AREA_NAME,"Etobicoke")
